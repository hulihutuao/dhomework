...
def 制作交叉表(数据表, 自变量, 因变量):
   ...
   def 计算单变量均值的置信水平(数据表，变量名，置信水平=0.95):
#打开数据文件
    file_path = 数据表路径及文件名
...
#计算置信区间(数据表，变量名，置信水平)
confidenc_interval = stats.t.interval(confidence_leval,自由度,loc=mean,scale=std_error)
#输出结果
print(F"均值：{mean: .2f}")
print(F"均值在置信水平：{confidence_leval}下的置信区间为：",confidenc_interval)